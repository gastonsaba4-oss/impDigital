...
import {
  ...
  Pencil, Trash2, Check, Scissors, Tags, Settings, LayoutDashboard,
  Image as ImageIcon,
} from "lucide-react";
    ...
    <article className="group flex flex-col">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-2.5 sm:p-3">
          <span className="inline-flex items-center gap-1 self-start rounded-full bg-white/90 text-stone-700 text-xs font-medium px-2 py-1">
      ...
      <div className="mt-2.5 px-0.5 flex-1 flex flex-col">
        ...
        <div className="mt-1.5">
          <span className="font-mono-data font-bold text-stone-900">{fmt(service.price, currency)}</span>
        </div>
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="bg-white border border-stone-200 rounded-3xl p-5 sm:p-7">
        <div className="flex items-center justify-between mb-1 gap-3">
          ...
          <span className="font-mono-data font-bold text-lg text-stone-900 shrink-0">{fmt(service.price, config.currency)}</span>
        </div>
...
export function AdminServiceForm({ service, categories, currency, onSave, onCancel }) {
  ...
  const [form, setForm] = useState(() =>
    service
      ? { ...service, price: String(service.price), durationMinutes: String(service.durationMinutes) }
      : { name: "", shortDescription: "", description: "", price: "", durationMinutes: "30", categoryId: categories[0] ? categories[0].id : "", image: "", active: true }
  );
        ...
        <div className="space-y-3">
          {sorted.map((a) => (
            <div key={a.id} className="bg-white border border-stone-200 rounded-2xl p-4">
              ...
              <p className="text-sm font-medium text-stone-800">{a.serviceName}</p>
