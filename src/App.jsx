...
function PublicStorePage() {
  ...
  const branding = site ? { name: site.name, paletteId: site.paletteId } : { name: "", paletteId: "verde-tierra" };
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-body text-stone-800" style={paletteCssVars(palette)}>
...
function PlatformPage() {
  ...
  async function onCreateSite(site) {
    ...
    const full = { ...site, clientCode, createdAt: new Date().toISOString() };
  ...
  function onUpdateSite(site) {
    const next = sites.map((s) => (s.id === site.id ? { ...s, name: site.name, paletteId: site.paletteId } : s));
    persistSites(next);
