    ...
    <div className={`min-h-screen bg-stone-50 font-body text-stone-800 ${branding.radiusStyle === "sharp" ? "radius-sharp" : ""}`} style={paletteCssVars(palette)}>
      ...
      {screen === "product" ? (
        ...
        <HomeScreen
          ...
          waLink={`https://wa.me/${normalizeWhatsAppDigits(config.whatsapp)}`}
          waReady={waReady}
        />
      )}
...
function ClientAdminPage() {
  ...
  function adminNavigate(screen, action) {
    if (screen === "products" && action === "new") {
      if (atProductLimit) { showToast(`Alcanzaste el límite de ${site.productLimit} productos de tu plan.`); setAdminScreen("products"); return; }
  ...
  function handleNewProduct() {
    if (atProductLimit) { showToast(`Alcanzaste el límite de ${site.productLimit} productos de tu plan.`); return; }
    setEditingProductId(null);
    setAdminScreen("product-form");
  }

  const waReady = !!(config.whatsapp && normalizeWhatsAppDigits(config.whatsapp).length >= 8);
  const atProductLimit = !!site && site.productLimit !== null && site.productLimit !== undefined && products.length >= site.productLimit;
